...
def fit(X, mu_low, mu_up, data_token=''):
  ...
  for i in range(n_steps):
    ...
    if args.save_dir:
      if args.lib == 'torch':
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'A_i{}.npy'.format(i)), A.cpu().numpy())
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'pi_i{}.npy'.format(i)), pi.cpu().numpy())
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'mu_i{}.npy'.format(i)), mu.cpu().numpy())
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'sigma_sqr_i{}.npy'.format(i)), sigma_sqr.cpu().numpy())
      else:
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'A_i{}.npy'.format(i)), A)
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'pi_i{}.npy'.format(i)), pi)
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'mu_i{}.npy'.format(i)), mu)
        np.save(os.path.join(SAVE_ROOT, args.save_dir, 'sigma_sqr_i{}.npy'.format(i)), sigma_sqr)
...
if __name__ == '__main__':
  ...
  if args.save_dir:
    os.makedirs(os.path.join(SAVE_ROOT, args.save_dir), exist_ok=True)
    os.makedirs(os.path.join(SAVE_ROOT, args.save_dir, 'figs'), exist_ok=True)
  else:
