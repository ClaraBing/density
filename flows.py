...
import data.flows_data as datasets
import flows as fnn
import utils
...
parser.add_argument(
    '--wd', type=float, default=1e-6, help='learning rate (default: 0.0001)')
parser.add_argument(
    '--dataset',
    default='POWER',
    help='POWER | GAS | HEPMASS | MINIBONE | BSDS300 | MOONS | SPHERE | LINE')
parser.add_argument('--d', type=int, help="Dimension for Sphere.")
parser.add_argument(
    '--model', default='maf', help='flow to use: maf | realnvp | glow')
parser.add_argument(
    '--no-cuda',
    action='store_true',
    default=False,
...
if args.cond:
    ...
    assert args.model in ['maf', 'realnvp'] and args.dataset == 'MNIST', \
    ...
    if args.dataset in ['SPHERE', 'LINE']:
      dataset = getattr(datasets, args.dataset)(args.d)
    else:
      dataset = getattr(datasets, args.dataset)()
...
num_hidden = {
    ...
    'MOONS': 64,
    'MNIST': 1024,
    'SPHERE': max(num_inputs*2, 32),
    'LINE': max(num_inputs*2, 32),
}[args.dataset]
...
assert args.model in ['maf', 'maf-split', 'maf-split-glow', 'realnvp', 'glow']
if args.model == 'glow':
...
def validate(epoch, model, loader, prefix='Validation'):
    ...
    wandb.log({
      'val/LL': val_loss / len(loader.dataset)
    })
